 ...
 * 建筑物高度可以增加的最大总和是多少？
 * 
 * 例子： 输入： grid = [[3,0,8,4],[2,4,5,7],[9,2,6,3],[0,3,1,0]] 输出： 35 解释： The grid
 * is: [ [3, 0, 8, 4], [2, 4, 5, 7], [9, 2, 6, 3], [0, 3, 1, 0] ]
 * 
...
public class Pro0807 {
	public int maxIncreaseKeepingSkyline(int[][] grid) {
        ...
        for (int r = 0; r < N; ++r)
            for (int c = 0; c < N; ++c) {
            	if(grid[r][c] > rowMaxes[r])
            		rowMaxes[r] = grid[r][c];
                if(grid[r][c] > colMaxes[c])
                	colMaxes[c] = grid[r][c];
        }
